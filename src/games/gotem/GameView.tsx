...
import { useGotemGameLogic } from "./useGameLogic";
import { usePlayerStore } from "@/hooks/usePlayerStore";
import { ScorePicker } from "@/components/ScorePicker";
import type { Segment, GotemThrowRecord, GotemState } from "@/lib/types";

interface GotemGameViewProps {
  playerIds: string[];
...
function ThrowBadge({ record }: { record: GotemThrowRecord }) {
  const hasEffect =
    record.marksAdded > 0 || record.pointsScored > 0 || record.claimed;
  return (
    <span
    ...
    >
      ...
      {record.claimed && <span className="text-xs">★</span>}
...
export function GotemGameView({
  ...
  const { state, registerThrow, endTurn, undo, reset } =
    useGotemGameLogic(playerIds);
  ...
  return (
    <div className="flex flex-1 flex-col items-center gap-6 py-6">
      <div className="flex w-full max-w-3xl items-center justify-between">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Got&apos;em</p>
      </div>
              ...
              >
                ...
                {state.players.map((p) => (
                  <td key={p.playerId} className="px-3 py-1.5">
                    <MarksDisplay
                      count={p.marks[target] ?? 0}
                      owned={owner !== null}
                      isOwner={owner === p.playerId}
                    />
                  </td>
                ))}
              </tr>
